...
class VungleAppOpenAdManager(
    ...
    companion object {
        const val TAG = "VungleAppOpenAd"
    }

    private var interstitialAd: InterstitialAd? = null

    fun loadAd() {
        Log.d(TAG, "Loading app open ad for placement: $placementId")
        
        interstitialAd = InterstitialAd(activity, placementId, AdConfig().apply {
            adOrientation = AdConfig.AUTO_ROTATE
        }).apply {
            adListener = this@VungleAppOpenAdManager
            load()
        }
    }

    fun showAd() {
        Log.d(TAG, "Showing app open ad for placement: $placementId")
        if (interstitialAd?.canPlayAd() == true) {
            interstitialAd?.play(activity)
        } else {
            Log.w(TAG, "App open ad cannot be played")
